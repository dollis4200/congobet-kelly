...
def run_scrape_cycle(db: dict, initial_bankroll: float, bankroll_current: float, cfg: dict) -> tuple[dict, pd.DataFrame, float]:
    ...
    snapshot = scraper.scrape_all(min_seconds=int(cfg['min_seconds_before_start']), include_results=need_history, include_standings=need_history)
...
def render_metrics(snapshot: dict, opp_df: pd.DataFrame, bankroll_current: float, initial_bankroll: float) -> None:
    ...
    c3.metric('🕒 Affiche ciblée', target.get('selected_label', '-'))
...
with st.sidebar:
    ...
    if st.button('Appliquer le bankroll manuel'):
...
if manual_cycle or auto_refresh:
    with st.spinner('Scraping CongoBet en cours...'):
        try:
            snapshot, opp_df, bankroll_current = run_scrape_cycle(db, initial_bankroll, bankroll_current, cfg)
            st.session_state['db'] = db
